...
enum SpinnerType1:Int {
    case dip = 0
    case LeBzul = 1
}

@IBDesignable class LBZSpinner1 : UIView, UITableViewDelegate, UITableViewDataSource {
    ...
    //spinner
    @IBInspectable var textFont: UIFont = UIFont.systemFont(ofSize: 16.0) { didSet{ updateUI() } }
    @IBInspectable var textColor: UIColor = UIColor.gray { didSet{ updateUI() } }
    @IBInspectable var lineColor: UIColor = UIColor.white { didSet{ updateUI() } }
    @IBInspectable var list: [String]  = [String]() { didSet{ updateUI() } }
    @IBInspectable var text: String = "" { didSet{ updateUI() } }
    
    //spinner type
    @IBInspectable var type: Int =  SpinnerType1.dip.rawValue { didSet{ updateUI() } }
    ...
    @IBInspectable var dDLStrokeColor: UIColor = UIColor.white
    @IBInspectable var dDLStrokeSize: CGFloat = 0
    ...
    var delegate:LBZSpinnerDelegate1!
    ...
    fileprivate func initCustomView() {
        ...
        //Open spinner click
        let gesture = UITapGestureRecognizer(target: self, action: #selector(LBZSpinner1.openSpinner(_:)))
    ...
    func changeSelectedIndex(_ index:Int) {
        if list.count > index {
            ...
            if (delegate != nil) {
                delegate.spinnerChoose1(self,index:selectedIndex, value: list[selectedIndex])
            }
        }
    }
    ...
    @objc func openSpinner(_ sender:UITapGestureRecognizer){
        ...
        let gesture = UITapGestureRecognizer(target: self, action: #selector(LBZSpinner1.closeSpinner))
    ...
    fileprivate func drawCanvas(frame: CGRect = CGRect(x: 0, y: 0, width: 86, height: 11)) {
        ...
        switch type {
        case SpinnerType1.dip.rawValue:
        ...
        case SpinnerType1.LeBzul.rawValue :
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if (delegate != nil) {
            delegate.spinnerChoose1(self,index: indexPath.row, value: list[indexPath.row])
        }
